...
// Parameters for sheets listing
interface ListSheetsParams {
  userId: string;
  pageSize?: number;
  pageToken?: string;
  sortBy?: 'name' | 'lastModified';
  sortOrder?: 'asc' | 'desc';
  query?: string; // For filtering by name
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const params = await req.json() as ListSheetsParams;
    if (!params.userId) {
      return new Response(
    ...
    let queryParams = ["mimeType='application/vnd.google-apps.spreadsheet'"];
    ...
    console.log("API returned data:", { fileCount: data.files?.length || 0 });
