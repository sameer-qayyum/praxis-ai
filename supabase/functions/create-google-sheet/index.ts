...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!userId || !sheetName) {
      return new Response(
        JSON.stringify({ error: "Missing required parameters: userId and sheetName" }),
    ...
    const newSheet: GoogleSheet = {
      ...
      lastModified: new Date().toISOString(), // The API doesn't return creation time, so we use current time
